...
const updateInfo = async ({
  ...
  return await userModel
    .findOneAndUpdate(
      query,
      avatarUrl === null ? updateSet : updateSetHasAvatar,
      options
    )
...
const updatePassword = async ({ User, password }) => {
  ...
  userUpdate = await foundUser.updateOne(
    { $set: { password: passwordHash, is_has_password: true } },
    { new: true }
  );
  if (userUpdate !== null) {
    return userUpdate;
...
const changePassword = async ({ userId, phone, password, newPassword }) => {
  ...
  const userUpdate = await userModel
    ...
    .select(
      unGetSelectData(["__v", "friends", "is_online", "last_seen", "password"])
    );
  if (userUpdate !== null) {
...
const resetPassword = async ({ phone, newPassword }) => {
  ...
  const userUpdate = await foundUser.updateOne(
...
const getUserBySearch = async ({ search, userId }) => {
  if (!search || typeof search !== "string")
    throw new Error("Invalid search term");

  if (/^\d{8,15}$/.test(search)) {
    const exactUser = await userModel
      .find({ phone: search })
      .lean();
  ...
  if (!foundUsers || foundUsers.length === 0)
    throw new NotFoundError("Không tìm thấy người phù hợp.");
  return foundUsers;
};
...
const sendFriendRequest = async ({ phone, friendId }) => {
  ...
  const existingRequest = receiver.friendRequests.find(
    (req) => req._id.toString() === sender._id.toString()
  );
  if (existingRequest)
    throw new BadRequestError("You have already sent a friend request");
...
const cancelFriendRequest = async ({ senderId, receiverId }) => {
  ...
  receiver.friendRequests = receiver.friendRequests.filter(
    (friendRequest) => friendRequest.toString() !== senderId
  );
...
const acceptFriendRequest = async ({ receiverId, senderId }) => {
  ...
  const isFriendForReceiver = receiver.friends.find(
  ...
  const isFriendForSender = sender.friends.find(
...
const unfriend = async ({ phone, friendId }) => {
  ...
  const isFriendForSender = sender.friends.some((friend) =>
  ...
  const isFriendForReceiver = receiver.friends.some((friend) =>
    friend.equals(sender._id)
  );
  if (!isFriendForReceiver)
    throw new BadRequestError("Receiver is not friends with you");
...
const checkSendFriendRequest = async ({ phone, friendId }) => {
  ...
  const isSentRequestForSender = sender.sentRequests.some((friend) =>
    friend.equals(friendId)
  );
...
const checkReceiveFriendRequest = async ({ phone, friendId }) => {
  ...
  const isReceiveRequestForReceiver = receiver.friendRequests.some((friend) =>
    friend.equals(friendId)
  );
...
const getSendFriendRequest = async ({ phone }) => {
  ...
  const sendFriendRequest = await userModel
    .find({ _id: { $in: findUser.sentRequests } })
    .lean();
...
const getReceiveFriendRequest = async ({ phone }) => {
  ...
  const receiveFriendRequest = await userModel
    .find({ _id: { $in: findUser.friendRequests } })
    .lean();
...
const getFriendList = async ({ phone }) => {
  ...
  const friendList = await userModel
    .find({ _id: { $in: findUser.friends } })
    .lean();
