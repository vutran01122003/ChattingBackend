...
class UserService {
    ...
    static editProfile = async ({ phone, fullName, dateOfBirth, gender, base64 }) => {
        const age = new Date().getFullYear() - new Date(dateOfBirth).getFullYear();
        if (age < 14) throw new BadRequestError('Age must be greater than 14');
    ...
    static updateAvatar = async ({ phone, avatarUrl }) => {
        const foundUser = await findUserByPhoneNumber({ phone, select: ['phone', 'full_name', 'is_has_password', 'avatar_url'] });
        if (!foundUser) throw new BadRequestError('Cannot find user')
